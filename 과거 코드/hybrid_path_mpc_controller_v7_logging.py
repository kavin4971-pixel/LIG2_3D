...
class MPCConfig:
    ...
    terminal_waypoint_lookahead: float = 8
    ...
    recapture_entry_distance: float = 4.5
    recapture_distance: float = 18.0
    ...
    turnback_forward_err_exit: float = 1.00
    turnback_yaw_exit: float = 0.70
    turnback_yaw_error_cmd: float = 1.25
    turnback_lateral_deadband: float = 0.20
    turnback_speed_cmd: float = 0.95
    turnback_speed_min: float = 0.82
    turnback_speed_max: float = 1.18
    turnback_prop_floor: float = 0.28
    turnback_yaw_kp_scale: float = 1.65
    turnback_pitch_kp_scale: float = 1.25
    capture_corridor_distance: float = 9.0
    capture_corridor_forward_min: float = -0.35
    capture_corridor_closing_min: float = 0.04
    capture_speed_min: float = 0.42
    capture_speed_max: float = 0.74
    capture_prop_floor: float = 0.18
    capture_yaw_kp_scale: float = 1.35
    capture_pitch_kp_scale: float = 1.18
    capture_current_comp_gain: float = 1.10
...
class PathGuidedSamplingMPC:
    ...
    def _recapture_mode(
        ...
        if best_dist > self.cfg.recapture_entry_distance:
            return False
    ...
    def _direct_goal_yaw_error(self, pos: np.ndarray, eta: Optional[np.ndarray] = None) -> float:
        eta_use = self.env.state[:6] if eta is None else np.asarray(eta, dtype=np.float64)
        goal_vec = self.env.target - pos
        if np.linalg.norm(goal_vec[:2]) < 1e-8:
            return 0.0
        desired_yaw, _ = course_to_angles(goal_vec)
        return float(wrap_angle(desired_yaw - eta_use[5]))

    def _capture_corridor_mode(
        self,
        pos: np.ndarray,
        eta: Optional[np.ndarray] = None,
        nu: Optional[np.ndarray] = None,
    ) -> bool:
        if not self._recapture_mode(pos, eta, nu):
            return False
        goal_dist = float(np.linalg.norm(self.env.target - pos))
        if goal_dist > self.cfg.capture_corridor_distance:
            return False
        rel_body = self._goal_relative_body(pos, eta)
        forward_err = float(rel_body[0])
        closing_speed = self._goal_closing_speed(pos, eta, nu)
        return bool(
            forward_err > self.cfg.capture_corridor_forward_min
            or closing_speed > self.cfg.capture_corridor_closing_min
        )
    ...
    def _turnback_mode(
        ...
        if not self._recapture_mode(pos, eta, nu):
            return False
        if self._capture_corridor_mode(pos, eta, nu):
            return False
        rel_body = self._goal_relative_body(pos, eta)
        forward_err = float(rel_body[0])
        yaw_error = abs(self._direct_goal_yaw_error(pos, eta))
        closing_speed = self._goal_closing_speed(pos, eta, nu)
        return bool(
            forward_err < self.cfg.turnback_forward_err_exit
            or yaw_error > self.cfg.turnback_yaw_exit
            or closing_speed < -0.04
        )
    ...
    def _turnback_sign(
        ...
        eta_use = self.env.state[:6] if eta is None else np.asarray(eta, dtype=np.float64)
        nu_use = self.env.state[6:] if nu is None else np.asarray(nu, dtype=np.float64)
        rel_body = self._goal_relative_body(pos, eta_use)
        lateral_err = float(rel_body[1])
        yaw_error = self._direct_goal_yaw_error(pos, eta_use)

        if abs(lateral_err) > self.cfg.turnback_lateral_deadband:
            sign = float(np.sign(lateral_err))
        elif abs(yaw_error) > 0.25:
            sign = float(np.sign(yaw_error))
        elif abs(nu_use[5]) > 0.03:
            sign = float(np.sign(nu_use[5]))
        else:
            sign = self.recapture_turn_sign

        if abs(sign) < 1e-8:
            sign = 1.0
        self.recapture_turn_sign = sign
        return sign
    ...
    def _reference_speed(self, pos: np.ndarray, eta: np.ndarray) -> float:
        ...
        capture_mode = self._capture_corridor_mode(pos, eta)
        turnback_mode = self._turnback_mode(pos, eta)

        if capture_mode:
            speed = min(self.cfg.capture_speed_max, 0.34 + 0.06 * goal_dist)
            if clear < self.cfg.clearance_slowdown_range:
                speed *= np.clip(clear / self.cfg.clearance_slowdown_range, 0.60, 1.0)
            if wall_margin < self.cfg.wall_recovery_margin:
                speed *= np.clip(wall_margin / self.cfg.wall_recovery_margin, 0.60, 1.0)
            return float(np.clip(speed, self.cfg.capture_speed_min, self.cfg.capture_speed_max))

        if turnback_mode:
            speed = self.cfg.turnback_speed_cmd
            if clear < self.cfg.clearance_slowdown_range:
                speed *= np.clip(clear / self.cfg.clearance_slowdown_range, 0.65, 1.0)
            if wall_margin < self.cfg.wall_recovery_margin:
                speed *= np.clip(wall_margin / self.cfg.wall_recovery_margin, 0.65, 1.0)
            min_speed = self.cfg.turnback_speed_min if clear > 1.2 and wall_margin > self.cfg.wall_safe_margin else self.cfg.wall_steer_speed_min
            return float(np.clip(speed, min_speed, self.cfg.turnback_speed_max))
    ...
    def _reference_angles(self, pos: np.ndarray, desired_speed: float) -> Tuple[float, float]:
        ...
        capture_mode = self._capture_corridor_mode(pos)
        turnback_mode = self._turnback_mode(pos)

        if capture_mode or turnback_mode:
            # Recovery is a direct re-approach, not a loiter/orbit behavior.
            # Keep current compensation near the goal so cross-current does not sweep
            # the vehicle past the capture sphere.
            to_goal = self.env.target - pos
            norm = np.linalg.norm(to_goal)
            if norm < 1e-8:
                return course_to_angles(to_goal)
            direct_ground = max(desired_speed, self.cfg.capture_speed_min) * (to_goal / norm)
            desired_rel = direct_ground - self.cfg.capture_current_comp_gain * current
            if np.linalg.norm(desired_rel) < 1e-6:
                desired_rel = direct_ground
        ...
        if goal_dist <= self.cfg.terminal_sampling_distance:
            min_comp_scale = 0.85 if (recapture_mode or self._terminal_mode(pos)) else 0.50
            comp_gain *= np.clip(goal_dist / max(self.cfg.terminal_sampling_distance, 1e-6), min_comp_scale, 1.0)
    ...
    def _dynamic_min_speed(self, pos: np.ndarray) -> float:
        ...
        wall_margin = boundary_margin(pos, self.env.world_size)
        clear = obstacle_clearance(pos, self.env.obstacles, self.env.auv_radius)
        if self._capture_corridor_mode(pos):
            return self.cfg.capture_speed_min
        if self._turnback_mode(pos):
            return self.cfg.turnback_speed_min if clear > 1.2 and wall_margin > self.cfg.wall_safe_margin else self.cfg.wall_steer_speed_min
    ...
    def _sample_action_sequences(self, mean: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self.cfg.cem_iters):
            ...
            prop_floor = 0.15
            if self._capture_corridor_mode(current_pos):
                prop_floor = self.cfg.capture_prop_floor
            elif self._turnback_mode(current_pos):
                prop_floor = self.cfg.turnback_prop_floor
            elif self._recapture_mode(current_pos):
    ...
    def _evaluate_action_sequence(self, sequence: np.ndarray) -> float:
        ...
        for action in sequence:
            ...
            forward_err = float(rel_body[0])
            recapture_mode = (
                best_rollout_goal <= self.cfg.recapture_entry_distance
            ...
            direct_yaw, _ = course_to_angles(target - pos)
            turnback_rollout = bool(
                recapture_mode
                and (
                    forward_err < self.cfg.turnback_forward_err_exit
                    or abs(wrap_angle(direct_yaw - eta[5])) > self.cfg.turnback_yaw_exit
                )
            )
            if dist_goal > self.cfg.near_goal_relax_distance:
                ...
                if recapture_mode:
                    ...
                    if turnback_rollout:
                        total_cost += 0.70 * max(0.0, self.cfg.turnback_prop_floor - float(action[0])) ** 2
                    else:
                        total_cost += self.cfg.recapture_closing_speed_weight * max(0.0, -closing_speed + 0.05) ** 2
                    if forward_err < self.cfg.recapture_reverse_trigger and not turnback_rollout:
    ...
    def _low_level_action(self, desired_speed: float, desired_yaw: float, desired_pitch: float) -> np.ndarray:
        ...
        capture_mode = self._capture_corridor_mode(eta[:3], eta, nu)
        turnback_mode = self._turnback_mode(eta[:3], eta, nu)
        vel_inertial = rotation_matrix_body_to_inertial(eta[3], eta[4], eta[5]) @ nu[:3]
        ...
        if capture_mode:
            speed_cap = min(speed_cap, self.cfg.capture_speed_max)
            speed_cap = max(speed_cap, self.cfg.capture_speed_min)
        elif turnback_mode:
            speed_cap = max(speed_cap, self.cfg.turnback_speed_min)
            speed_cap = min(speed_cap, self.cfg.turnback_speed_max)
        elif recapture_mode:
        ...
        recover_mode = recapture_mode or capture_mode or turnback_mode or goal_dist <= self.cfg.terminal_capture_distance or missed_capture or wall_margin < self.cfg.wall_recovery_margin
        yaw_kp = self.cfg.yaw_kp * (self.cfg.capture_yaw_kp_scale if capture_mode else self.cfg.turnback_yaw_kp_scale if turnback_mode else 1.30 if recapture_mode else 1.20 if recover_mode else 1.0)
        pitch_kp = self.cfg.pitch_kp * (self.cfg.capture_pitch_kp_scale if capture_mode else self.cfg.turnback_pitch_kp_scale if turnback_mode else 1.10 if goal_dist <= self.cfg.terminal_capture_distance else 1.0)
        ...
        if capture_mode:
            propeller = max(propeller, self.cfg.capture_prop_floor)
        elif turnback_mode:
            propeller = max(propeller, self.cfg.turnback_prop_floor)
        elif recapture_mode:
    ...
    def act(self) -> np.ndarray:
        ...
        clear = obstacle_clearance(pos, self.env.obstacles, self.env.auv_radius)
        wall_margin = boundary_margin(pos, self.env.world_size)
        sampling_needed = self._sampling_needed(pos)
        capture_mode = self._capture_corridor_mode(pos)
        turnback_mode = self._turnback_mode(pos)
        recapture_mode = self._recapture_mode(pos)
        terminal_mode = self._terminal_mode(pos)
        missed_capture = self._missed_capture(pos)
        wall_recovery_mode = self._wall_recovery_mode(pos)
        ...
        controller_debug = {
            ...
            "ref_pitch": float(ref_pitch),
            "current_body_est": self._current_from_profile(self.env.step_count).astype(float).tolist(),
            "nominal_action": nominal_action.astype(float).tolist(),
            "sampling_used": bool(sampling_needed),
            "path_len": int(len(self.cached_path)),
            "waypoint": (self.cached_waypoint if len(self.cached_path) > 0 else self.env.target).astype(float).tolist(),
            "modes": {
                "capture": bool(capture_mode),
                "turnback": bool(turnback_mode),
                "recapture": bool(recapture_mode),
                "terminal": bool(terminal_mode),
                "missed_capture": bool(missed_capture),
                "wall_recovery": bool(wall_recovery_mode),
            },
        }

        if not sampling_needed:
            self.cached_actions = self._warm_start_mean(nominal_action)
            action_out = np.asarray(nominal_action, dtype=np.float32)
            controller_debug.update(
                {
                    "sampled_action": None,
                    "blend_nominal": 1.0,
                    "command_action": action_out.astype(float).tolist(),
                }
            )
            self.last_debug = controller_debug
            return action_out
        ...
        if capture_mode:
            ...
            blend_nominal = 0.82
        elif terminal_mode or missed_capture:
            blend_nominal = 0.65
        elif wall_margin < self.cfg.wall_recovery_margin:
            blend_nominal = 0.60
        else:
            blend_nominal = 0.75
        ...
        action_out = np.asarray(blended, dtype=np.float32)
        controller_debug.update(
            {
                "sampled_action": best_sequence[0].astype(float).tolist(),
                "blend_nominal": float(blend_nominal),
                "command_action": action_out.astype(float).tolist(),
            }
        )
        self.last_debug = controller_debug
            ...
            control_log.append(
                {
                    ...
                    "time": float(env.step_count * env.dt),
                    "position": env.state[:3].astype(float).tolist(),
                    "attitude": env.state[3:6].astype(float).tolist(),
                    "body_velocity": env.state[6:].astype(float).tolist(),
                    "command_action": np.asarray(action, dtype=np.float64).tolist(),
                    "actuator_state": np.asarray(step_info.get("actuator_state", env.actuator_state), dtype=np.float64).tolist(),
                    "distance_to_goal": float(step_info.get("distance_to_goal", np.linalg.norm(env.state[:3] - env.target))),
                    "clearance": float(obstacle_clearance(env.state[:3], env.obstacles, env.auv_radius)),
                    "wall_margin": float(boundary_margin(env.state[:3], env.world_size)),
                    "current_body": np.asarray(step_info.get("current_body", controller._current_from_profile(env.step_count)), dtype=np.float64).tolist(),
                    "reward": float(reward),
                    "event": str(step_info.get("event", "")),
                    "controller": dict(controller.last_debug),
                }
            )
