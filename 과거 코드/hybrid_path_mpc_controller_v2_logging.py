...
class MPCConfig:
    ...
    terminal_capture_distance: float = 12.0
    terminal_sampling_distance: float = 15.0
    terminal_speed_max: float = 1.40
    terminal_min_speed: float = 0.35
    terminal_waypoint_lookahead: float = 10.0
    wall_recovery_margin: float = 6.0
    wall_safe_margin: float = 4.5
    wall_outward_speed_weight: float = 22.0
    shortcut_boundary_margin: float = 6.0
    shortcut_clearance_margin: float = 2.0
    shortcut_max_factor: float = 1.35
...
class PathGuidedSamplingMPC:
    ...
    def _reference_speed(self, pos: np.ndarray, eta: np.ndarray) -> float:
        ...
        if goal_dist <= self.cfg.terminal_capture_distance:
            # Dedicated terminal-capture schedule: slow enough to settle inside the
            # goal ball, but not so slow that current prevents closing.
            speed = min(self.cfg.terminal_speed_max, 0.45 + 0.08 * goal_dist)
            ...
            if wall_margin < self.cfg.wall_recovery_margin:
                speed *= np.clip(wall_margin / self.cfg.wall_recovery_margin, 0.35, 1.0)
            required = self._required_body_speed(pos, self.env.step_count)
            speed = max(speed, min(self.cfg.terminal_speed_max, required))
        ...
        # open, then taper off only when obstacles or the boundary demand it.
        ...
        if wall_margin < max(self.cfg.boundary_influence, self.cfg.wall_recovery_margin):
            speed *= np.clip(wall_margin / max(self.cfg.boundary_influence, self.cfg.wall_recovery_margin), 0.40, 1.0)
    ...
    def _dynamic_min_speed(self, pos: np.ndarray) -> float:
        ...
        clear = obstacle_clearance(pos, self.env.obstacles, self.env.auv_radius)
        if goal_dist <= self.cfg.terminal_capture_distance:
            return self.cfg.terminal_min_speed
        if wall_margin < self.cfg.wall_recovery_margin or clear < 1.2:
            return min(self.cfg.min_speed_cmd, 0.45)
    ...
    def _evaluate_action_sequence(self, sequence: np.ndarray) -> float:
        ...
        for action in sequence:
            ...
            if dist_goal > self.cfg.near_goal_relax_distance:
                ...
                terminal_target_speed = min(self.cfg.terminal_speed_max, 0.35 + 0.08 * dist_goal)
                total_cost += 0.12 * (float(action[0]) - min(1.0, terminal_target_speed / max(self.cfg.max_speed_cmd, 1e-6))) ** 2
                if dist_goal > self.env.goal_radius:
                    total_cost += 2.5 * max(0.0, 0.25 - closing_speed) ** 2
    ...
    def _low_level_action(self, desired_speed: float, desired_yaw: float, desired_pitch: float) -> np.ndarray:
        ...
        if wall_margin < max(self.cfg.boundary_influence, self.cfg.wall_recovery_margin):
            speed_cap *= np.clip(wall_margin / max(self.cfg.boundary_influence, self.cfg.wall_recovery_margin), 0.28, 1.0)
        if wall_margin < self.cfg.wall_recovery_margin and outward_speed > 0.05:
            speed_cap *= 1.0 / (1.0 + 0.9 * outward_speed)
        ...
        yaw_kp = self.cfg.yaw_kp * (1.20 if goal_dist <= self.cfg.terminal_capture_distance else 1.0)
    ...
    def _maybe_replan_path(self) -> None:
        ...
        if clear > self.cfg.clearance_slowdown_range and goal_dist > 2.0 * self.cfg.waypoint_lookahead and wall_margin > self.cfg.shortcut_boundary_margin:
            lookahead = min(1.25 * self.cfg.waypoint_lookahead, self.cfg.waypoint_lookahead + 0.08 * goal_dist)
        if wall_margin < self.cfg.wall_recovery_margin:
            lookahead = min(lookahead, 0.75 * self.cfg.waypoint_lookahead)
