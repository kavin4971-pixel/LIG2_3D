...
def maybe_print_progress(
    *,
    ...
    print(
        ...
        f"| t={act_time*1000.0:5.1f} ms | R={episode_return:8.2f}",
        flush=True,
    )
...
class PathGuidedSamplingMPC:
    ...
    def _reference_speed(self, pos: np.ndarray, eta: np.ndarray) -> float:
        ...
        capture_latched = self._capture_latch_mode(pos)

        if final_capture:
            speed = 0.55 + 0.18 * min(horiz_err, 1.5) + 0.10 * min(abs(vert_err), 2.0)
            if clear < self.cfg.clearance_slowdown_range:
                speed *= np.clip(clear / self.cfg.clearance_slowdown_range, 0.70, 1.0)
        ...
        if capture_latched:
            speed = 0.74 + 0.05 * min(goal_dist, 5.0) + 0.06 * min(abs(vert_err), 2.5)
            if forward_err < 0.0:
                speed = max(speed, 0.86)
            if horiz_err < 1.6 and abs(vert_err) > 0.9:
                speed = min(speed, 0.88)
            if clear < self.cfg.clearance_slowdown_range:
                speed *= np.clip(clear / self.cfg.clearance_slowdown_range, 0.75, 1.0)
            if wall_margin < self.cfg.wall_recovery_margin:
                speed *= np.clip(wall_margin / self.cfg.wall_recovery_margin, 0.70, 1.0)
            return float(np.clip(speed, self.cfg.capture_latch_speed_min, self.cfg.capture_latch_speed_max))
    ...
    def _reference_angles(self, pos: np.ndarray, desired_speed: float) -> Tuple[float, float]:
        ...
        if final_capture:
            horiz_vec = rel.copy()
            horiz_vec[2] = 0.0
            horiz_norm = float(np.linalg.norm(horiz_vec[:2]))
            if horiz_norm > max(self.cfg.final_capture_xy_window, 1e-6):
                yaw_ref = float(np.arctan2(horiz_vec[1], horiz_vec[0]))
            else:
                yaw_ref = float(self.env.state[5])
            pitch_ref = float(np.clip(-np.arctan2(rel[2], max(horiz_norm, 0.75)), -0.45, 0.45))
        ...
        if self._capture_latch_mode(pos):
            ...
            horiz_vec[2] = 0.0
            horiz_norm = float(np.linalg.norm(horiz_vec[:2]))
            yaw_ref = float(np.arctan2(horiz_vec[1], horiz_vec[0])) if horiz_norm > 1e-6 else float(self.env.state[5])
            pitch_ref = float(np.clip(-np.arctan2(rel[2], max(horiz_norm, 0.80)), -0.45, 0.45))
            return float(wrap_angle(yaw_ref)), pitch_ref
    ...
    def _low_level_action(self, desired_speed: float, desired_yaw: float, desired_pitch: float) -> np.ndarray:
        ...
        goal_dist, horiz_err, vert_err, _ = self._goal_error_components(eta[:3])
        ...
        capture_latched = self._capture_latch_mode(eta[:3])
        vel_inertial = rotation_matrix_body_to_inertial(eta[3], eta[4], eta[5]) @ nu[:3]
        ...
        elif capture_latched:
            speed_cap = float(np.clip(desired_speed, self.cfg.capture_latch_speed_min, self.cfg.capture_latch_speed_max))
            if horiz_err < 1.6 and abs(vert_err) > 0.9:
                speed_cap = min(speed_cap, 0.88)
        ...
        recover_mode = goal_dist <= self.cfg.terminal_capture_distance or missed_capture or wall_margin < self.cfg.wall_recovery_margin
        ...
        if capture_latched:
            yaw_kp *= self.cfg.capture_latch_yaw_gain
            pitch_kp *= self.cfg.capture_latch_pitch_gain
        if final_capture:
            yaw_kp *= 0.70
        ...
        if capture_latched and forward_err < 0.0:
            propeller = max(propeller, self.cfg.capture_latch_prop_floor + 0.05)
        ...
        if capture_latched and horiz_err < 1.6 and abs(vert_err) > 0.9 and not final_capture:
            rudder = float(np.clip(rudder, -0.35, 0.35))
            stern *= 1.15
        if final_capture:
            if horiz_err <= self.cfg.final_capture_xy_window:
    ...
    def _maybe_replan_path(self) -> None:
        ...
        clear = obstacle_clearance(pos, self.env.obstacles, self.env.auv_radius)
        wall_margin = boundary_margin(pos, self.env.world_size)
        if self._capture_latch_mode(pos) or goal_dist <= self.cfg.terminal_capture_distance or self._missed_capture(pos):
    ...
    def act(self) -> np.ndarray:
        ...
        if self._terminal_mode(pos) or self._missed_capture(pos):
            blend_nominal = 0.65
