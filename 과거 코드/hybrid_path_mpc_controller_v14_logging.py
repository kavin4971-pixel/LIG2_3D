...
class MPCConfig:
    ...
    vertical_priority_distance: float = 5.5
    vertical_priority_xy_window: float = 1.45
    vertical_priority_z_threshold: float = 0.75
    vertical_priority_speed_min: float = 0.92
    vertical_priority_speed_max: float = 1.18
    vertical_priority_prop_floor: float = 0.34
    vertical_priority_rudder_limit: float = 0.34
    vertical_priority_pitch_gain: float = 1.60
    vertical_priority_xy_scale: float = 0.28
    vertical_priority_current_comp_gain: float = 0.10
    vertical_priority_z_weight: float = 8.0
...
class PathGuidedSamplingMPC:
    ...
    def _reference_speed(self, pos: np.ndarray, eta: np.ndarray) -> float:
        ...
        vertical_priority = self._vertical_priority_mode(pos, eta)
        ...
        elif vertical_priority:
            speed = 1.00 + 0.08 * min(abs(vert_err), 2.5) + 0.02 * min(horiz_err, 1.0)
            speed -= 0.04 * v_t
            speed -= 0.06 * max(0.0, -v_r)
            if clear < self.cfg.clearance_slowdown_range:
                speed *= np.clip(clear / self.cfg.clearance_slowdown_range, 0.82, 1.0)
            if wall_margin < self.cfg.wall_recovery_margin:
                speed *= np.clip(wall_margin / self.cfg.wall_recovery_margin, 0.82, 1.0)
            return float(np.clip(speed, self.cfg.vertical_priority_speed_min, self.cfg.vertical_priority_speed_max))
    ...
    def _vertical_priority_mode(self, pos: np.ndarray, eta: Optional[np.ndarray] = None) -> bool:
        if self.recapture_active:
            return False
        goal_dist, horiz_err, vert_err, _ = self._goal_error_components(pos)
        if goal_dist > self.cfg.vertical_priority_distance:
            return False
        if horiz_err > self.cfg.vertical_priority_xy_window:
            return False
        if abs(vert_err) < self.cfg.vertical_priority_z_threshold:
            return False
        clear = obstacle_clearance(pos, self.env.obstacles, self.env.auv_radius)
        wall_margin = boundary_margin(pos, self.env.world_size)
    ...
    def _reference_angles(self, pos: np.ndarray, desired_speed: float) -> Tuple[float, float]:
        ...
        vertical_priority = self._vertical_priority_mode(pos)
        ...
        if vertical_priority:
            desired_vec = rel.copy()
            desired_vec[:2] *= self.cfg.vertical_priority_xy_scale
            if v_t > 1e-6:
                desired_vec = desired_vec - 0.85 * tangential_vec
            if v_r < 0.0:
                desired_vec = desired_vec + 0.80 * (-v_r) * (rel / max(goal_dist, 1e-6))
            desired_vec = desired_vec - self.cfg.vertical_priority_current_comp_gain * current
            if np.linalg.norm(desired_vec) < 1e-8:
                desired_vec = rel.copy()
            return course_to_angles(desired_vec)
    ...
    def _dynamic_min_speed(self, pos: np.ndarray) -> float:
        ...
        if self._vertical_priority_mode(pos):
            return self.cfg.vertical_priority_speed_min
    ...
    def _evaluate_action_sequence(self, sequence: np.ndarray, max_cost: float = np.inf) -> float:
        ...
        for action in sequence:
            ...
            if self._vertical_priority_mode(pos, eta):
                total_cost += self.cfg.vertical_priority_z_weight * (vert_err ** 2)
                total_cost += 0.6 * self.cfg.capture_tangential_weight * (v_t ** 2)
                total_cost += 0.25 * (horiz_err ** 2)
            if capture_commit:
    ...
    def _low_level_action(self, desired_speed: float, desired_yaw: float, desired_pitch: float) -> np.ndarray:
        ...
        vertical_priority = self._vertical_priority_mode(eta[:3], eta)
        ...
        if vertical_priority:
            yaw_kp *= 0.72
            pitch_kp *= self.cfg.vertical_priority_pitch_gain
        if final_capture:
            yaw_kp *= 1.05
        ...
        if vertical_priority:
            prop_floor = max(prop_floor, self.cfg.vertical_priority_prop_floor)
            propeller = max(propeller, self.cfg.vertical_priority_prop_floor + 0.06)
            propeller = min(propeller, min(0.82, self.cfg.vertical_priority_speed_max))
        if final_capture:
        ...
        if capture_commit:
            ...
            if abs(vert_err) > 0.9:
                stern *= 1.12
        if vertical_priority:
            rudder = float(np.clip(rudder, -self.cfg.vertical_priority_rudder_limit, self.cfg.vertical_priority_rudder_limit))
            stern *= 1.18
            if abs(vertical_err) > self.cfg.vertical_priority_z_threshold:
                stern_bias = np.clip(0.22 * np.sign(vertical_err), -0.28, 0.28)
    ...
    def _sampling_needed(self, pos: np.ndarray) -> bool:
        ...
        if self.recapture_active and clear > (self.cfg.obstacle_influence + 1.0) and wall_margin > (self.cfg.wall_recovery_margin + 1.0):
            return False
        if self._vertical_priority_mode(pos) and clear > (self.cfg.obstacle_influence + 1.0) and wall_margin > (self.cfg.wall_recovery_margin + 1.0):
            return False
        if self._final_capture_mode(pos):
    ...
    def act(self) -> np.ndarray:
        ...
        elif self._vertical_priority_mode(pos, eta):
            self.current_mode = "VPRIO"
