...
class PathGuidedSamplingMPC:
    ...
    def _reference_angles(self, pos: np.ndarray, desired_speed: float) -> Tuple[float, float]:
        ...
        if turnback_mode:
            ...
            yaw_error = self._direct_goal_yaw_error(pos, eta)
            _, direct_pitch = course_to_angles(self.env.target - pos)
            if abs(yaw_error) > self.cfg.turnback_yaw_exit:
                yaw_cmd = wrap_angle(eta[5] + self._turnback_sign(pos, eta) * self.cfg.turnback_yaw_error_cmd)
            else:
                yaw_cmd = wrap_angle(eta[5] + yaw_error)
            return yaw_cmd, direct_pitch
    ...
    def _sample_action_sequences(self, mean: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self.cfg.cem_iters):
            ...
            prop_floor = 0.15
            if self._turnback_mode(current_pos):
    ...
    def _low_level_action(self, desired_speed: float, desired_yaw: float, desired_pitch: float) -> np.ndarray:
        ...
        recover_mode = recapture_mode or turnback_mode or goal_dist <= self.cfg.terminal_capture_distance or missed_capture or wall_margin < self.cfg.wall_recovery_margin
        yaw_kp = self.cfg.yaw_kp * (self.cfg.turnback_yaw_kp_scale if turnback_mode else 1.30 if recapture_mode else 1.20 if recover_mode else 1.0)
        pitch_kp = self.cfg.pitch_kp * (self.cfg.turnback_pitch_kp_scale if turnback_mode else 1.10 if goal_dist <= self.cfg.terminal_capture_distance else 1.0)
