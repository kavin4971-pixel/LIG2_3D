...
class MPCConfig:
    ...
    final_capture_distance: float = 2.6
    final_capture_xy_window: float = 1.20
    final_capture_speed_min: float = 0.70
    final_capture_speed_max: float = 0.92
    final_capture_prop_floor: float = 0.24
    final_capture_rudder_limit: float = 0.28
    final_capture_pitch_gain: float = 1.35
    final_capture_stern_bias: float = 0.22
    final_capture_current_comp_gain: float = 0.0
    ...
    recapture_reentry_speed_min: float = 0.95
    recapture_reentry_speed_max: float = 1.28
    recapture_reentry_prop_floor: float = 0.32
    recapture_reentry_rudder_limit: float = 0.72
    recapture_anchor_radius: float = 1.75
    recapture_goal_release_distance: float = 4.2
    recapture_rearm_steps: int = 40
...
class PathGuidedSamplingMPC:
    ...
    def _activate_recapture_anchor(self, pos: np.ndarray, eta: Optional[np.ndarray] = None, nu: Optional[np.ndarray] = None) -> None:
        goal_dist, _, vert_err, rel, _, _, v_t, tangential_vec = self._goal_velocity_components(pos, eta, nu)
        outward_hat = self._safe_unit(pos - self.env.target)
        if np.linalg.norm(outward_hat) < 1e-8:
            outward_hat = -self._safe_unit(rel)
        tangent_hat = self._safe_unit(tangential_vec)
        anchor = self.env.target.copy() - self.cfg.recapture_reentry_distance * outward_hat
        lateral = self.cfg.recapture_reentry_lateral_scale * min(v_t, 1.0)
        if np.linalg.norm(tangent_hat) > 1e-8:
            anchor = anchor - lateral * tangent_hat
        anchor[2] = np.clip(self.env.target[2] + 0.35 * np.clip(vert_err, -4.0, 4.0), 1.0, self.env.world_size - 1.0)
        anchor[:2] = np.clip(anchor[:2], -self.env.world_size + 3.0, self.env.world_size - 3.0)
        anchor[2] = np.clip(anchor[2], 1.0, self.env.world_size - 1.0)
        self.recapture_anchor = anchor
    ...
    def _navigation_target(self) -> np.ndarray:
        if self.recapture_active:
            return self.recapture_anchor.copy()
        if self.cached_path:
    ...
    def _mpc_profile(self, pos: np.ndarray) -> Tuple[int, int, int]:
        ...
        if self._capture_latch_mode(pos) or goal_dist < self.cfg.terminal_sampling_distance or missed or clear < 6.0 or wall_margin < self.cfg.wall_recovery_margin + 0.5:
    ...
    def _reference_speed(self, pos: np.ndarray, eta: np.ndarray) -> float:
        ...
        if final_capture:
            speed = 0.72 + 0.06 * min(abs(vert_err), 2.0)
            speed -= 0.22 * v_t
            speed -= 0.38 * max(0.0, -v_r)
            if clear < self.cfg.clearance_slowdown_range:
    ...
    def _final_capture_mode(self, pos: np.ndarray, eta: Optional[np.ndarray] = None) -> bool:
        ...
        if horiz_err <= self.cfg.final_capture_xy_window:
            return True
        return bool(goal_dist <= 2.0 * self.env.goal_radius and abs(vert_err) >= 0.60)
    ...
    def _reference_angles(self, pos: np.ndarray, desired_speed: float) -> Tuple[float, float]:
        ...
        if self.recapture_active:
            desired_vec = self.recapture_anchor - pos
            if goal_dist < 10.0 and v_t > 1e-6:
                desired_vec = desired_vec - 0.35 * tangential_vec
            if goal_dist < 10.0 and v_r < 0.0:
                desired_vec = desired_vec + 0.45 * (-v_r) * (rel / max(goal_dist, 1e-6))
            if self._wall_recovery_mode(pos):
                wall_push = self._wall_push_vector(pos)
                if np.linalg.norm(wall_push) > 1e-8:
                    desired_vec = desired_vec + 0.65 * max(desired_speed, 0.9) * wall_push
        ...
        if final_capture or capture_commit:
            ...
            if v_t > 1e-6:
                desired_vec = desired_vec - self.cfg.capture_tangent_cancel_gain * tangential_vec
            if v_r < 0.0:
                desired_vec = desired_vec + self.cfg.capture_outward_cancel_gain * (-v_r) * (rel / max(goal_dist, 1e-6))
            if final_capture and horiz_err <= self.cfg.final_capture_xy_window:
                yaw_ref = float(self.env.state[5])
                pitch_ref = float(np.clip(-np.arctan2(rel[2], max(horiz_err, 0.70)), -0.50, 0.50))
                return float(wrap_angle(yaw_ref)), pitch_ref
            if horiz_err <= self.cfg.capture_commit_xy_hold_window and abs(vert_err) > 0.9:
                desired_vec[:2] *= 0.55
            if np.linalg.norm(desired_vec) < 1e-8:
    ...
    def _dynamic_min_speed(self, pos: np.ndarray) -> float:
        ...
        clear = obstacle_clearance(pos, self.env.obstacles, self.env.auv_radius)
        if self.recapture_active:
            return self.cfg.recapture_reentry_speed_min
    ...
    def _low_level_action(self, desired_speed: float, desired_yaw: float, desired_pitch: float) -> np.ndarray:
        ...
        anchor_dist = float(np.linalg.norm(self.recapture_anchor - eta[:3])) if recapture_active else np.inf
        ...
        if final_capture:
            if horiz_err <= self.cfg.final_capture_xy_window:
                rudder = float(np.clip(rudder, -self.cfg.final_capture_rudder_limit, self.cfg.final_capture_rudder_limit))
    ...
    def _maybe_replan_path(self) -> None:
        ...
        wall_margin = boundary_margin(pos, self.env.world_size)
        if self.recapture_active:
            self.cached_path = [self.recapture_anchor.copy(), self.env.target.copy()]
            self.cached_waypoint = self.recapture_anchor.copy()
